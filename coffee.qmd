...
### Spending Habits
As one would expect, as one becomes more experiences, one spends more money in a hobby. That was very true for me as I fell into black hole of striving to perfect my coffee game, wanting to buy the top gear given my budget.

For the folks with less expertise, nearly half (48%) of them spend less than \$20 a month. This suggests this group probably doesn't drink a lot of specialty coffee since a bag costs \$10-20 a bag nor drinks coffee at specialty cafes.
...
I wonder if this group has a coffee preference, one akin to Folger's
...
df |> filter(what_is_your_favorite_coffee_drink != "", !is.na(exp_group)) |> count(what_is_your_favorite_coffee_drink, exp_group) |> group_by(exp_group) |> mutate(perc = n / sum(n)) |> gg_bars("exp_group", "perc", single_color = TRUE, bar_color = "steelblue") + facet_wrap(~what_is_your_favorite_coffee_drink, ncol = 6)

df |> filter(!is.na(do_you_usually_add_anything_to_your_coffee_no_just_black), !is.na(exp_group)) |> count(do_you_usually_add_anything_to_your_coffee_no_just_black, exp_group) |> group_by(exp_group) |> mutate(perc = n / sum(n)) |> gg_bars("exp_group", "perc", single_color = TRUE, bar_color = "steelblue") + facet_grid(~do_you_usually_add_anything_to_your_coffee_no_just_black)
```
```{r}
#| fig-height: 2.5
...
Most of us have probably seen tasting notes when purchasing coffee (i.e. grapefruit, chocolate) but are these actually descriptive? Would someone describe similar notes of the same coffee? Is there a relationship between tasting notes and roast levels? To find this out, I looked at how participants ranked the level of bitterness and acidity of each coffee. Then they were asked to describe their tasting notes so I will explore this via a text analysis.
...
```{r ttests}
#| fig-subcap: "conf.level = 0.95, statistical significant at the 5 percent levels"
...
To start, participants were asked to rate the level of bitterness and acidity of each coffee from a scale of 1 -5. Average bitterness among the four coffees show that [Coffee A]{.coffeea} and [Coffee D]{.coffeed} were similar while [Coffee B]{.coffeeb} and [Coffee C]{.coffeec} were also similar. But are these averages between these two pair of coffee statistically significant. To find this out, I performed a one-sided t-test. show that the average may be different (p = xx).

Again, for acidity, we have similar patterns.

### Flavor Notes

Cool we found a difference in bitterness and acidity in the coffees! So ***What does the coffee remind you of?*** was a repeated question James asked to describe the flavor notes. Comparing coffees side-by-side allows one to discover the nuances of each coffee. One may start with a general words such as 'nutty.' Below is a flavor wheel, a tool to help identify words for one's taste buds.
