...
:::: {.columns}
::: {.column width="45%"}
I also looked at the breakdown of daily coffee drinking by age in @tbl-age_habit. Interestingly, we see a trend in drinking more coffee per day as one ages. Who said college students drink lots of coffee, maybe we should say it's the retirees who drink far more because of their spare time?
:::
::: {.column width="55%"}
...
df |> count(gender_relabel) |> mutate(perc = n / sum(n)) |> ggplot(aes(reorder(gender_relabel, n), perc, fill = gender_relabel)) + geom_bar(stat = 'identity') + coord_flip() + theme_coffee(base_size = 16) + geom_text(
...
:::: {.columns}
::: {.column width="50%"}
Let's explore the coffee drinking habits by expertise group. @fig-drink_exp shows a trend in increasing coffee consumption as one learns more about coffee. Some even drinking 4 or more cups per day with nearly 1 out of 10 participants for the '9-10' expertise group. There's a similar pattern with drinking coffee just black - meaning no milk, sugar, or other additive (like olive oil).
:::
::: {.column width="50%"}
```{r}
...
We find similar a pattern in low spending when asked how much a participant spent on coffee equipment over the past five years. While James says having a good coffee grinder can greatly improve a cup of coffee, a quality one typically costs more than $100. This implies these folks probably make terrible coffee with a drip coffee maker, but who am I to judge?
...
Participants tasted the coffee via coffee cupping, a coffee tasting technique where a taster takes and spoonful of coffee a slurps it as to asses aspects such as cleanness, sweetness, acidity, mouthfeel and aftertaste as in the above picture. But why the slurp? Slurping aerates the coffee as it makes contact with taste buds which intensifies the tasting sensation. Swishing it over the whole mouth coats all areas of the tongue, the front sweet, sides sour and back bitter as to assess all aspects of taste.
...
Participants were asked to rate the level of **bitterness** and **acidity** of each coffee on a scale of 1-5. @tbl-bitterstats shows that a rank of 2, 3, 3, and 2 were the top choices for Coffees A-D, respectively. @tbl-bitterstats demonstrates that average bitterness of [Coffee A]{.coffeea} and [Coffee D]{.coffeed} were similar while [Coffee B]{.coffeeb} and [Coffee C]{.coffeec} were also similar. TO find if these averages between these two pair of coffee are statistically significant, I performed a one-sided t-test.

show that the average may be different (p = xx).
...
bitter_table =
  left_join(mean_bitter, bitter_dist, by = "Coffee") |> gt() |> gt_theme_538() |> gt_add_coffee_color("Coffee") |> gt::fmt_number() |> cols_move(c("Average Bitterness", "Standard Deviation"), after = "Coffee") |> gtExtras::gt_plt_dist(
    list_data,
    type = "histogram",
    line_color = "white",
    fill_color = "steelblue",
...
Comparing coffees side-by-side allows one to discover the nuances of each coffee. One may start with a general words such as 'nutty' but then move on to identify which nut. Below is a coffee flavor wheel, a tool to help identify words for one's taste buds (note this was NOT used during the live event).
...
Participants were asked to describe the coffee flavors in an open text field. After some cleaning of these words, I performed a text analysis by identifying and counting key flavor words, such as 'bright' or 'grapefruit.' I took the **top 25 most frequent words** of each coffee and put them in a **word cloud.** The larger the word, the frequent the word was mentioned (note: the count of each word is in the hover text).

<div id="parent">
<div id="child-left">
<p class="wc_cap">[Coffee A]{.coffeea}: Top Word was **Fruity** with 616 Respondents</p>
<iframe src = "/assets/widgets/a.html" width = "100%" height = "200"></iframe></div>
<div id="child-right">
<p class="wc_cap">[Coffee B]{.coffeeb}: Top Word was **Chocolate** with 600 Respondents</p>
<iframe src = "/assets/widgets/b.html" width = "100%" height = "200"></iframe></div>
</div>

<div id="parent">
<div id="child-left">
<p class="wc_cap">[Coffee C]{.coffeec}: Top Word was **Chocolate** with 332 Respondents</p>
<iframe src = "/assets/widgets/c.html" width = "100%" height = "200"></iframe></div>
<div id="child-right">
<p class="wc_cap">[Coffee D]{.coffeed}: Top Word was **Fruity** with 1,085 Respondents</p>
<iframe src = "/assets/widgets/d.html" width = "100%" height = "200"></iframe></div>
</div>

[Coffee A]{.coffeea} conjures up images of something *light*, *bright* and *fruity.* While participants described [Coffee B]{.coffeeb} and [Coffee C]{.coffeec} as *chocolate*, *nutty*, and *balanced* while a few even described it as *burnt*. [Coffee C]{.coffeec} had more of a spread in word counts as words are similar in size. [Coffee D]{.coffeed}, however, paints a much different image. *Fermented* and *funky* made it to the top - given this was a fermented natural processed coffee, this is not surprising!

B and C

Okay - so let's see how these tasting notes match up on the flavor wheel below. ['Acid', 'Bitter', 'Sweet', 'Fruity',]{.coffeeother} and ['Chocolate']{.coffeeother} are generic words used to describe all 4 coffees, though at different frequencies as demonstrated in the word clouds. Looking at the yellow sectors, we do see a pattern for [Coffee A]{.coffeea} and [Coffee D]{.coffeed} compared to [Coffee B]{.coffeeb} and [Coffee C]{.coffeec}.
...
#| fig-cap: "The Colors Represent Which Taste Notes Overlap Which Coffee"
...
```
Out of 
Coffee D is also ones who love pour overs, not lattes. Generically more for lighter roasts. Seems like Coffee C is the loser - the most bitter (maybe that's just my bias)
As we've previously seen, *chocolate* was describe in Coffee B and Coffee C so it's not surprising to see their favorite coffee was either of these two. We also see a correlation between those groups who liked fruity and those favorite was Coffee D. 
```{r}
#| label: tbl-heatmap
...
## Conclusion
This cupping live stream demonstrates there's actually some commonalities between tasting by the cupping participants. Coffee A and Coffee D were both fruity, while Coffee D was described as 'fermented'. Coffee B and Coffee C also had similar flavor notes, which Coffee C was statistically described as being more bitter, on average. This also demonstrated there's not much flavor differences in medium and dark roasts as compared to light roast coffees.